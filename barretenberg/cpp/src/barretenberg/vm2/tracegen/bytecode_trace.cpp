    ...
    TraceContainer& trace)
{
    ...
    for (const auto& [bytecode_id, event_indices] : bytecode_id_to_events) {
        for (const auto& idx : event_indices) {
            ...
            auto bytecode_at = [&](size_t i) -> uint8_t {
                return i < event.bytecode->size() ? (*event.bytecode)[i] : 0;
            };

            const FF is_last_of_bytecode = event_indices.back() == idx ? 1 : 0;
            ...
            const uint32_t bytes_remaining =
                event.error == PC_OUT_OF_RANGE ? 0 : static_cast<uint32_t>(event.bytecode->size()) - event.pc;
            ...
            trace.set(row,
                      { {
                          ...
                          { C::instr_fetching_last_of_bytecode, is_last_of_bytecode },
                          ...
                          { C::instr_fetching_sel_lookup_bc_decomposition, event.error == PC_OUT_OF_RANGE ? 0 : 1 },
                      } });
            row++;
        }
    }
